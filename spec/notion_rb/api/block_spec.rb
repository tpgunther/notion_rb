...
RSpec.describe NotionRb::Api::Block do
  subject { NotionRb::Api::Block.new(notion_id: 'f0d7f6e4-c228-4cba-b860-a6f40ed3372e') }

  context '#response' do
    context 'success request' do
      it 'has 200 status', :vcr do
        expect(subject.send(:response).code).to eq '200'
      end
    end

    context 'non existing page' do
      subject { NotionRb::Api::Block.new(notion_id: 'non-existing-id') }
    ...
    context 'unexpected server error' do
      subject { NotionRb::Api::Block.new(notion_id: 'other-id') }
